...
- [x] **v0.9.7 — Code audit batch 2: data-loss fixes.** Targeted guard/condition fixes for 7 findings from the full multi-agent code audit (batch 1, the timecode bugs, shipped as v0.9.6):
  - **Cross-project result bleed (`store.ts` `updateDeliverables`, `applyResults`, `fillLocationsFromGraphics`):** async analysis runs capture their target project at start but previously wrote results into whatever project was *currently* active on completion. `updateDeliverables` now takes an optional `expectedProjectId` — every call site threads through the project id captured at the start of its run, and a stale/mismatched write is dropped with a console warning instead of silently landing in the wrong project.
  - **Boundary stitch pass deleting distinct dialogue/fauna entries:** the chunk-boundary shot-stitch pass (designed for shots/graphics spans that can legitimately straddle a chunk boundary) was also running on `dialogue_list`/`fauna_log`, where two distinct entries landing within 3s of each other near a boundary are common and real — restricted to `shot_list`/`graphics_list`.
  - **Overlap dedup killing real dialogue repeats:** the chunk-overlap duplicate-line dedup matched on text within a 30s window, wide enough to also delete genuine quick repeats (e.g. "Blow!" ×5) that happened to fall in the re-scan zone. Tightened to 6s — real chunk-boundary duplicates (same utterance seen by both overlapping chunks) land within a couple seconds of each other, not 30.
  - **Talent duration guard dropping recoverable people:** the global duration guard dropped a whole talent bio if `firstAppearance` was out of range, even when the person's `appearances` array had other valid in-range timestamps. Now recovers via the earliest valid appearance before falling back to dropping.
  - **Casing-defeated talent first-name rescue:** the v0.9.5 rescue's authority name set mixed casing ("SCOTT CARVER" from one chunk vs "Scott Carver" from lower-thirds), so the same person occupied two Set entries and made an unambiguous first name look ambiguous, defeating the rescue. Authority names are now title-cased at insertion so they collapse to one entry.
  - **Lower-third talent cross-reference — contamination + dead on EDL:** two bugs in the same block. (1) Substring matching (`content.includes(namePart)`) let a short name token match inside an unrelated person's lower-third text, cross-contaminating `firstAppearance`; now requires every name token to match a whole word in the lower-third's first line. (2) The block compared lower-third TCs (10h EDL record base when graphics came from the EDL path) directly against `durationSec` (0-based) — always false, so the cross-reference was a silent no-op on any EDL project. Now normalizes via the EDL start offset before comparing/merging.
  - **Two-pass shot list cancel committing a blank list:** cancelling mid-Phase-4 (describe) broke the description loop but still fell through to Phase 5 and committed a shot list with real cut TCs but blank descriptions, overwriting the clean slate `clearDeliverable` had set. Added the same cancel-flag guard already used before the EDL path's final `applyResults`.
  - **Upload-cancel mixing two videos (`UploadPanel.tsx`):** `selectFile` committed the newly-selected file's blob URL to the store immediately on drag/select — before the user clicked Upload. Clicking Cancel left the store's `videoBlobUrl` pointed at an unuploaded file while `project.videoFile` still referenced the previously uploaded one, so any thumbnail/two-pass frame extraction pulled frames from the wrong video. The blob URL is now only committed in `handleUpload`, at the point the user actually commits to the file.
  - **Held, not resolved by this batch:** Pro Mode visibility, fauna recall threshold tuning, the dead `/api/analyze` route — flagged during the audit but out of scope here.
